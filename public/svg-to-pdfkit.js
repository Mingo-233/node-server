var SVGtoPDF = function (doc, svg, x, y, options) {
        ...
        if (parser.match(/^>/)) {
          ...
          if ((temp = parser.match(/^<\/([\w:.-]+)\s*>/, true))) {
            ...
            if (temp[1] === node.nodeName) {
              ...
              warningCallback(
                'parseXml: tag not matching, opening "' +
                  node.nodeName +
                  '" & closing "' +
                  temp[1] +
                  '"'
          ...
          warningCallback(
            'parseXml: tag could not be parsed "' + node.nodeName + '"'
          );
          error = true;
        }
  ...
  function parseColor(raw) {
    ...
    if ((temp = NamedColors[raw])) {
      ...
      (temp = raw.match(/^#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i))
    ) {
      result = [
        [parseInt(temp[1], 16), parseInt(temp[2], 16), parseInt(temp[3], 16)],
        1,
      ];
    } else if ((temp = raw.match(/^#([0-9a-f])([0-9a-f])([0-9a-f])$/i))) {
      result = [
        [
          0x11 * parseInt(temp[1], 16),
          0x11 * parseInt(temp[2], 16),
          0x11 * parseInt(temp[3], 16),
        ],
        1,
      ];
    }
  ...
  function getPageBBox() {
    return new SvgShape()
      .M(0, 0)
      .L(doc.page.width, 0)
      .L(doc.page.width, doc.page.height)
      .L(0, doc.page.height)
  ...
  function getAscent(font, size) {
    return (
      (Math.max(
        font.ascender,
        (font.bbox[3] || font.bbox.maxY) * (font.scale || 1)
      ) *
        size) /
      1000
    );
  ...
  function getDescent(font, size) {
    return (
      (Math.min(
        font.descender,
        (font.bbox[1] || font.bbox.minY) * (font.scale || 1)
      ) *
        size) /
      1000
    );
  ...
  function getXHeight(font, size) {
    return (
      ((font.xHeight || 0.5 * (font.ascender - font.descender)) * size) / 1000
    );
  }
  ...
  function getTextPos(font, size, text) {
    ...
    for (let i = 0; i < hex.length; i++) {
      let unicode = font.unicode
        ? font.unicode[parseInt(hex[i], 16)]
        : [text.charCodeAt(i)];
      data.push({
        glyph: hex[i],
        unicode: unicode,
  ...
  var SvgShape = function () {
    ...
    this.S = function (c1x, c1y, x, y) {
      return this.C(
        currX + (lastCom === "C" ? currX - lastCtrlX : 0),
        currY + (lastCom === "C" ? currY - lastCtrlY : 0),
        c1x,
        c1y,
        x,
        y
    ...
    this.T = function (x, y) {
      return this.Q(
        currX + (lastCom === "Q" ? currX - lastCtrlX : 0),
        currY + (lastCom === "Q" ? currY - lastCtrlY : 0),
        x,
        y
      );
    };
    ...
    this.A = function (rx, ry, fi, fa, fs, x, y) {
      if (isEqual(rx, 0) || isEqual(ry, 0)) {
        ...
        for (let i = 0; i < segms; i++) {
          ...
          let c1x =
              cx +
              Math.cos(fi) * rx * (Math.cos(th3) - t * Math.sin(th3)) -
              Math.sin(fi) * ry * (Math.sin(th3) + t * Math.cos(th3)),
            c1y =
              cy +
              Math.sin(fi) * rx * (Math.cos(th3) - t * Math.sin(th3)) +
              Math.cos(fi) * ry * (Math.sin(th3) + t * Math.cos(th3)),
            c2x =
              cx +
              Math.cos(fi) * rx * (Math.cos(th4) + t * Math.sin(th4)) -
              Math.sin(fi) * ry * (Math.sin(th4) - t * Math.cos(th4)),
            c2y =
              cy +
              Math.sin(fi) * rx * (Math.cos(th4) + t * Math.sin(th4)) +
              Math.cos(fi) * ry * (Math.sin(th4) - t * Math.cos(th4)),
            endX =
              cx +
              Math.cos(fi) * rx * Math.cos(th4) -
              Math.sin(fi) * ry * Math.sin(th4),
            endY =
              cy +
              Math.sin(fi) * rx * Math.cos(th4) +
              Math.cos(fi) * ry * Math.sin(th4);
          this.addCommand([
            "curve",
            i === 0,
            i === segms - 1,
    ...
    this.path = function (d) {
      ...
      while ((command = parser.match(/^[astvzqmhlcASTVZQMHLC]/))) {
        ...
        let values = [];
        while (
          (value = PathFlags[command + values.length]
            ? parser.match(/^[01]/)
            : parser.matchNumber())
        ) {
          parser.matchSeparator();
  ...
  var SvgElem = function (obj, inherits) {
    ...
    this.inherits =
      inherits ||
      (!this.isOuterElement ? createSVGElement(obj.parentNode, null) : null);
    this.stack = this.inherits ? this.inherits.stack.concat(obj) : [obj];
    this.style = parseStyleAttr(
      typeof obj.getAttribute === "function" && obj.getAttribute("style")
    );
        ...
        if (!file) {
          ...
          if (svgObj) {
            if (this.stack.indexOf(svgObj) === -1) {
              ...
              warningCallback(
                'SVGtoPDF: loop of circular references for id "' + id + '"'
              );
              return;
            }
          }
          ...
          for (let i = 0; i < svgs.length; i++) {
            ...
            if (svgObj) {
              if (this.stack.indexOf(svgObj) === -1) {
                ...
                warningCallback(
                  'SVGtoPDF: loop of circular references for id "' +
                    file +
                    "#" +
                    id +
                    '"'
    ...
    this.computeUnits = function (value, unit, percent, isFontSize) {
      if (unit === "%") {
        return (
          ...
          (isFontSize || percent != null ? percent : this.getViewport())
        );
      } else if (unit === "ex" || unit === "em") {
        return (
          value *
          { em: 1, ex: 0.5 }[unit] *
          (isFontSize ? percent : this.get("font-size"))
        );
      } else {
        return (
          value *
    ...
    this.computeLength = function (value, percent, initial, isFontSize) {
      let parser = new StringParser((value || "").trim()),
        ...
        temp2;
      if (
        typeof (temp1 = parser.matchNumber()) === "string" &&
        typeof (temp2 = parser.matchLengthUnit()) === "string" &&
        !parser.matchAll()
      ) {
    ...
    this.get = function (key) {
          ...
          switch (key) {
            case "font-size":
              result = this.computeLength(
                value,
                this.inherits ? this.inherits.get(key) : keyInfo.initial,
                undefined,
                true
              );
              break;
      ...
      return (styleCache[key] =
        keyInfo.inherit && this.inherits
          ? this.inherits.get(key)
          : keyInfo.initial);
    };
    this.getChildren = function () {
      ...
      for (let i = 0; i < obj.childNodes.length; i++) {
        ...
        if (
          !child.error &&
          this.allowedChildren.indexOf(child.nodeName) !== -1
        ) {
    ...
    this.getParentViewport = function () {
      return Math.sqrt(
        0.5 * this.getParentVWidth() * this.getParentVWidth() +
          0.5 * this.getParentVHeight() * this.getParentVHeight()
      );
    };
    ...
    this.getViewport = function () {
      return Math.sqrt(
        0.5 * this.getVWidth() * this.getVWidth() +
          0.5 * this.getVHeight() * this.getVHeight()
      );
    };
    this.getBoundingBox = function () {
  ...
  var SvgElemStylable = function (obj, inherits) {
    ...
    this.getStroke = function (isClip, isMask) {
      ...
      if (stroke !== "none" && opacity && strokeOpacity) {
        if (
          stroke instanceof SvgElemGradient ||
          stroke instanceof SvgElemPattern
        ) {
          return stroke.getPaint(
            this.getBoundingBox(),
  ...
  var SvgElemSymbol = function (obj, inherits) {
    ...
    let width = this.getLength(
      ...
      height = this.getLength(
        "height",
        this.getParentVHeight(),
        this.getParentVHeight()
      );
    if (inherits instanceof SvgElemUse) {
      width = inherits.getLength("width", inherits.getParentVWidth(), width);
      height = inherits.getLength(
        "height",
        inherits.getParentVHeight(),
        height
      );
    }
  ...
  var SvgElemLink = function (obj, inherits) {
    ...
    this.addLink = function () {
      if (
        this.link.match(/^(?:[a-z][a-z0-9+.-]*:|\/\/)?/i) &&
        this.getChildren().length
      ) {
        let bbox = this.getBoundingShape()
    ...
    this.drawInDocument = function (isClip, isMask) {
      ...
      if (this.attr("data-clip")) {
        ...
        new SvgShape()
          ...
          .L(x + tempLength + width, y + height + tempLength)
          .L(x - tempLength, y + tempLength + height)
          .Z()
          .transform(this.get("transform"))
          .insertInDocument();
        doc.clip();
  ...
  var SVGElemImage = function (obj, inherits) {
    SvgElemStylable.call(this, obj, inherits);
    let link = imageCallback(
        this.attr("href") || this.attr("xlink:href") || ""
      ),
  ...
  var SvgElemPattern = function (obj, inherits, fallback) {
    ...
    this.getPaint = function (bBox, gOpacity, isClip, isMask) {
      let bBoxUnitsPattern = this.attr("patternUnits") !== "userSpaceOnUse",
        bBoxUnitsContent =
          this.attr("patternContentUnits") === "objectBoundingBox",
        x = this.getLength(
          "x",
          bBoxUnitsPattern ? 1 : this.getParentVWidth(),
          0
        ),
        y = this.getLength(
          "y",
          bBoxUnitsPattern ? 1 : this.getParentVHeight(),
          0
        ),
      ...
      if (
        ...
        (aspectRatioMatrix = validateMatrix(aspectRatioMatrix)) &&
  ...
  var SvgElemGradient = function (obj, inherits, fallback) {
    ...
    this.getPaint = function (bBox, gOpacity, isClip, isMask) {
      ...
      if (children.length === 1) {
        ...
        return opacityToColor(
          stopColor,
          child.get("stop-opacity") * gOpacity,
          isMask
        );
      }
      ...
      if ((matrix = validateMatrix(matrix))) {
        ...
        if (spread === "reflect" || spread === "repeat") {
          ...
          if (this.name === "linearGradient") {
            ...
            nAfter =
              Math.max(
                (corner1[0] - x2) * (x2 - x1) + (corner1[1] - y2) * (y2 - y1),
            ...
            nBefore =
              Math.max(
                (corner1[0] - x1) * (x1 - x2) + (corner1[1] - y1) * (y1 - y2),
            ...
            nAfter =
              Math.sqrt(
                Math.max(
                  Math.pow(corner1[0] - x2, 2) + Math.pow(corner1[1] - y2, 2),
            ...
            for (let j = 0; j < subPaths.length; j++) {
              if (isEqual(subPaths[j].totalLength, 0)) {
                if (
                  ...
                  if (
                    subPaths[j].startPoint &&
                    subPaths[j].startPoint.length > 1
                  ) {
        ...
        if (
          ...
          if (markerEnd !== "none" && markersPos.length > 0) {
            ...
            marker.drawMarker(
              false,
              isMask,
              markersPos[markersPos.length - 1],
              lineWidth
            );
          }
        }
  ...
  var SvgElemPath = function (obj, inherits) {
    ...
    this.dashScale =
      this.pathLength !== undefined
        ? this.shape.totalLength / this.pathLength
        : 1;
  };
  ...
  var SvgElemMarker = function (obj, inherits) {
    ...
    this.drawMarker = function (isClip, isMask, posArray, strokeWidth) {
      ...
      let refX = this.getLength("refX", this.getVWidth(), 0),
        ...
        aspectRatioMatrix = parseAspectRatio(
          this.attr("preserveAspectRatio"),
          width,
          height,
          viewBox[2],
      ...
      if (this.get("overflow") === "hidden") {
        doc
          .rect(
            aspectRatioMatrix[0] * (viewBox[0] + viewBox[2] / 2 - refX) -
              width / 2,
            aspectRatioMatrix[3] * (viewBox[1] + viewBox[3] / 2 - refY) -
              height / 2,
            width,
            height
          )
    ...
    this.useMask = function (bBox) {
      ...
      if (this.attr("maskUnits") === "userSpaceOnUse") {
        x = this.getLength(
          "x",
          this.getVWidth(),
          -0.1 * (bBox[2] - bBox[0]) + bBox[0]
        );
        y = this.getLength(
          "y",
          this.getVHeight(),
          -0.1 * (bBox[3] - bBox[1]) + bBox[1]
        );
        w = this.getLength(
          "width",
          this.getVWidth(),
        ...
        x =
          this.getLength("x", this.getVWidth(), -0.1) * (bBox[2] - bBox[0]) +
          bBox[0];
        y =
  ...
  var SvgElemTextContainer = function (obj, inherits) {
    ...
    this.drawTextInDocument = function (isClip, isMask) {
      ...
      if (this.get("text-decoration") === "underline") {
        this.decorate(
          0.05 * this._font.size,
          -0.075 * this._font.size,
          isClip,
          isMask
        );
      }
      if (this.get("text-decoration") === "overline") {
        this.decorate(
          0.05 * this._font.size,
          getAscent(this._font.font, this._font.size) + 0.075 * this._font.size,
          isClip,
          isMask
        );
      }
      ...
      if (this.get("text-decoration") === "line-through") {
        this.decorate(
          0.05 * this._font.size,
          0.5 *
            (getAscent(this._font.font, this._font.size) +
              getDescent(this._font.font, this._font.size)),
          isClip,
          isMask
        );
      }
  ...
  var SvgElemTextPath = function (obj, inherits) {
    ...
    if ((temp = this.attr("path")) && temp.trim() !== "") {
      ...
      this.pathLength =
        pathLength > 0 ? pathLength : this.pathObject.totalLength;
      this.pathScale = this.pathObject.totalLength / this.pathLength;
    } else if (
      (temp = this.getUrl("href") || this.getUrl("xlink:href")) &&
      temp.nodeName === "path"
    ) {
      let pathElem = new SvgElemPath(temp, this);
      this.pathObject = pathElem.shape
        .clone()
        .transform(pathElem.get("transform"));
      this.pathLength = this.chooseValue(
        pathElem.pathLength,
  ...
  var SvgElemText = function (obj, inherits) {
    ...
    (function (textParentElem) {
      ...
      function recursive(currentElem, parentElem) {
        currentElem._x = combineArrays(
          currentElem.getLengthList("x", currentElem.getVWidth()),
          parentElem ? parentElem._x.slice(parentElem._pos.length) : []
        );
        currentElem._y = combineArrays(
          currentElem.getLengthList("y", currentElem.getVHeight()),
          parentElem ? parentElem._y.slice(parentElem._pos.length) : []
        );
        currentElem._dx = combineArrays(
          currentElem.getLengthList("dx", currentElem.getVWidth()),
          parentElem ? parentElem._dx.slice(parentElem._pos.length) : []
        );
        currentElem._dy = combineArrays(
          currentElem.getLengthList("dy", currentElem.getVHeight()),
          parentElem ? parentElem._dy.slice(parentElem._pos.length) : []
        );
        currentElem._rot = combineArrays(
          currentElem.getNumberList("rotate"),
          parentElem ? parentElem._rot.slice(parentElem._pos.length) : []
        );
        currentElem._defRot = currentElem.chooseValue(
          currentElem._rot[currentElem._rot.length - 1],
          parentElem && parentElem._defRot,
          0
        );
        ...
        let fontOptions = { fauxItalic: false, fauxBold: false },
          fontNameorLink = fontCallback(
            ...
            currentElem.get("font-weight") === "bold",
        ...
        let textLength = currentElem.getLength(
            "textLength",
            currentElem.getVWidth(),
            undefined
          ),
          spacingAndGlyphs =
            currentElem.attr("lengthAdjust") === "spacingAndGlyphs",
      ...
      function textOnPath(currentElem) {
        ...
        if (pathObject) {
          ...
          for (let j = 0; j < currentElem._pos.length; j++) {
            let charMidX =
              textOffset +
              currentElem._pos[j].x +
              0.5 * currentElem._pos[j].width;
            if (charMidX > pathLength || charMidX < 0) {
              currentElem._pos[j].hidden = true;
            } else {
              let pointOnPath = pathObject.getPointAtLength(
                charMidX * pathScale
              );
              ...
              currentElem._pos[j].x =
                pointOnPath[0] -
                0.5 * currentElem._pos[j].width * Math.cos(pointOnPath[2]) -
                currentElem._pos[j].y * Math.sin(pointOnPath[2]);
              currentElem._pos[j].y =
                pointOnPath[1] -
                0.5 * currentElem._pos[j].width * Math.sin(pointOnPath[2]) +
                currentElem._pos[j].y * Math.cos(pointOnPath[2]);
              currentElem._pos[j].rotate =
                pointOnPath[2] + currentElem._pos[j].rotate;
  ...
  if (svg) {
    ...
    if (typeof elem.drawInDocument === "function") {
      ...
      warningCallback(
        "SVGtoPDF: this element can't be rendered directly: " + svg.nodeName
      );
    }
  } else {
